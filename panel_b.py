...
def make_map( filename ):
    ...
    face_plane = d20.faces[5].plane
    rot_mat = mappings.face_dict['f'][0]
    ...
    for lat in range( 10, 170, 1 ):
        ...
        try:
            point = line_plane_intersect( two_point_line( zero_3d, _sphere_to_cart( lat_rad, 5*_pi/6 ) ), face_plane )
            mp1 = point.multiply( rot_mat )
            point = line_plane_intersect( two_point_line( zero_3d, _sphere_to_cart( lat_rad, _pi ) ), face_plane )
            mp2 = point.multiply( rot_mat )
            point = line_plane_intersect( two_point_line( zero_3d, _sphere_to_cart( lat_rad, 7*_pi/6 ) ), face_plane )
